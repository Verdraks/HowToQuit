...
Shader "ASE_Phantom"
{
	Properties
	{
		...
		_ColorInterior("ColorInterior", Color) = (1,1,1,0)
		_ColorOutline("ColorOutline", Color) = (1,1,1,0)
		_ColorSpread("ColorSpread", Float) = 0
	...
	SubShader
	{
		...
		Pass
		{
						 ...
						 ) : SV_Target
			{
				...
				float fresnelNdotV10 = dot( WorldNormal, ase_viewDirWS );
				float fresnelNode10 = ( 0.0 + 1.0 * pow( 1.0 - fresnelNdotV10, _PowerFresnel ) );
				float3 ase_objectPosition = GetAbsolutePositionWS( UNITY_MATRIX_M._m03_m13_m23 );
				float3 ase_viewDirOS = normalize( WorldViewDirection );
				float3 temp_output_26_0 = ( saturate( ( distance( ( ase_objectPosition - ase_viewDirOS ) , float3( 0,0,0 ) ) - _ColorSpread ) ) * _ColorInterior.rgb );
				float time31 = 1.32;
				float2 voronoiSmoothId31 = 0;
				float mulTime32 = _TimeParameters.x * _TimeScale;
				float2 temp_cast_0 = (mulTime32).xx;
				float2 texCoord33 = input.ase_texcoord8.xy * float2( 1,1 ) + temp_cast_0;
				float2 temp_output_34_0_g1 = ( texCoord33 - float2( 0.5,0.5 ) );
				float2 break39_g1 = temp_output_34_0_g1;
				float2 appendResult50_g1 = (float2(( _RadialScale * ( length( temp_output_34_0_g1 ) * 2.0 ) ) , ( ( atan2( break39_g1.x , break39_g1.y ) * ( 1.0 / TWO_PI ) ) * 1.0 )));
				float2 coords31 = appendResult50_g1 * 1.5;
				float2 id31 = 0;
				float2 uv31 = 0;
				float fade31 = 0.5;
				float voroi31 = 0;
				float rest31 = 0;
				for( int it31 = 0; it31 <3; it31++ ){
				voroi31 += fade31 * voronoi31( coords31, time31, id31, uv31, 0,voronoiSmoothId31 );
				rest31 += fade31;
				coords31 *= 2;
				fade31 *= 0.5;
				}//Voronoi31
				...
				float3 BaseColor = ( ( fresnelNode10 * _ColorOutline.rgb ) + ( temp_output_26_0 + ( temp_output_26_0 * step( voroi31 , 0.23 ) ) ) );
		...
		Pass
		{
						 ...
						 ) : SV_Target
			{
				...
				float fresnelNdotV10 = dot( ase_normalWS, ase_viewDirWS );
				float fresnelNode10 = ( 0.0 + 1.0 * pow( 1.0 - fresnelNdotV10, _PowerFresnel ) );
		...
		Pass
		{
			...
			half4 frag(PackedVaryings input  ) : SV_Target
			{
				...
				float mulTime32 = _TimeParameters.x * _TimeScale;
				float2 temp_cast_0 = (mulTime32).xx;
				float2 texCoord33 = input.ase_texcoord5.xy * float2( 1,1 ) + temp_cast_0;
				float2 temp_output_34_0_g1 = ( texCoord33 - float2( 0.5,0.5 ) );
				float2 break39_g1 = temp_output_34_0_g1;
				float2 appendResult50_g1 = (float2(( _RadialScale * ( length( temp_output_34_0_g1 ) * 2.0 ) ) , ( ( atan2( break39_g1.x , break39_g1.y ) * ( 1.0 / TWO_PI ) ) * 1.0 )));
				float2 coords31 = appendResult50_g1 * 1.5;
		...
		Pass
		{
			...
			half4 frag(PackedVaryings input  ) : SV_Target
			{
				...
				float mulTime32 = _TimeParameters.x * _TimeScale;
				float2 temp_cast_0 = (mulTime32).xx;
				float2 texCoord33 = input.ase_texcoord3.xy * float2( 1,1 ) + temp_cast_0;
				float2 temp_output_34_0_g1 = ( texCoord33 - float2( 0.5,0.5 ) );
				float2 break39_g1 = temp_output_34_0_g1;
				float2 appendResult50_g1 = (float2(( _RadialScale * ( length( temp_output_34_0_g1 ) * 2.0 ) ) , ( ( atan2( break39_g1.x , break39_g1.y ) * ( 1.0 / TWO_PI ) ) * 1.0 )));
				float2 coords31 = appendResult50_g1 * 1.5;
		...
		Pass
		{
			...
			half4 frag(PackedVaryings input ) : SV_Target
			{
				...
				surfaceDescription.Alpha = fresnelNode10;
...
/*ASEBEGIN
Version=19800
Node;AmplifyShaderEditor.RangedFloatNode;13;-752,96;Inherit;False;Property;_PowerFresnel;PowerFresnel;0;0;Create;True;0;0;0;False;0;False;0;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.ObjectPositionNode;20;-1040,528;Inherit;False;0;4;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3
Node;AmplifyShaderEditor.ViewDirInputsCoordNode;19;-1024,672;Inherit;False;Object;False;0;4;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3
Node;AmplifyShaderEditor.SimpleSubtractOpNode;21;-816,528;Inherit;False;2;0;FLOAT3;0,0,0;False;1;FLOAT3;0,0,0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.DistanceOpNode;22;-640,528;Inherit;False;2;0;FLOAT3;0,0,0;False;1;FLOAT3;0,0,0;False;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;25;-592,656;Inherit;False;Property;_ColorSpread;ColorSpread;3;0;Create;True;0;0;0;False;0;False;0;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.SimpleSubtractOpNode;23;-464,528;Inherit;False;2;0;FLOAT;0;False;1;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.FresnelNode;10;-528,16;Inherit;False;Standard;WorldNormal;ViewDir;False;False;5;0;FLOAT3;0,0,1;False;4;FLOAT3;0,0,0;False;1;FLOAT;0;False;2;FLOAT;1;False;3;FLOAT;5;False;1;FLOAT;0
Node;AmplifyShaderEditor.ColorNode;12;-368,-208;Inherit;False;Property;_ColorOutline;ColorOutline;2;0;Create;True;0;0;0;False;0;False;1,1,1,0;1,1,1,0;True;True;0;6;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4;FLOAT3;5
Node;AmplifyShaderEditor.SaturateNode;24;-304,528;Inherit;False;1;0;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.ColorNode;28;-352,608;Inherit;False;Property;_ColorInterior;ColorInterior;1;0;Create;True;0;0;0;False;0;False;1,1,1,0;1,1,1,0;True;True;0;6;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4;FLOAT3;5
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;26;-128,560;Inherit;False;2;2;0;FLOAT;0;False;1;FLOAT3;0,0,0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.RangedFloatNode;14;848,16;Inherit;False;Property;_AlphaClip;AlphaClip;4;0;Create;True;0;0;0;False;0;False;0.2;0.2;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;34;102.3226,554.6791;Inherit;False;2;2;0;FLOAT3;0,0,0;False;1;FLOAT;0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;11;0,-64;Inherit;False;2;2;0;FLOAT;0;False;1;FLOAT3;0,0,0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.SimpleAddOpNode;27;272,-80;Inherit;False;2;2;0;FLOAT3;0,0,0;False;1;FLOAT3;0,0,0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.SimpleAddOpNode;36;320,384;Inherit;False;2;2;0;FLOAT3;0,0,0;False;1;FLOAT3;0,0,0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.VoronoiNode;31;-112,816;Inherit;True;0;0;1;1;3;False;1;False;False;False;4;0;FLOAT2;0,0;False;1;FLOAT;1.32;False;2;FLOAT;1.5;False;3;FLOAT;0;False;3;FLOAT;0;FLOAT2;1;FLOAT2;2
Node;AmplifyShaderEditor.StepOpNode;37;96,816;Inherit;True;2;0;FLOAT;0;False;1;FLOAT;0.23;False;1;FLOAT;0
Node;AmplifyShaderEditor.FunctionNode;38;-352,816;Inherit;False;Polar Coordinates;-1;;1;7dab8e02884cf104ebefaa2e788e4162;0;4;1;FLOAT2;0,0;False;2;FLOAT2;0.5,0.5;False;3;FLOAT;1;False;4;FLOAT;1;False;3;FLOAT2;0;FLOAT;55;FLOAT;56
Node;AmplifyShaderEditor.TextureCoordinatesNode;33;-608,848;Inherit;False;0;-1;2;3;2;SAMPLER2D;;False;0;FLOAT2;1,1;False;1;FLOAT2;0,0;False;5;FLOAT2;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.SimpleTimeNode;32;-848,880;Inherit;False;1;0;FLOAT;1;False;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;39;-912,960;Inherit;False;Property;_TimeScale;TimeScale;5;0;Create;True;0;0;0;False;0;False;0;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;40;-592,1008;Inherit;False;Property;_RadialScale;RadialScale;6;0;Create;True;0;0;0;False;0;False;0;0;0;0;0;1;FLOAT;0
...
ASEEND*/
//CHKSM=A222A4A76317EDC8E42A3731E53C431FDF72AEAE